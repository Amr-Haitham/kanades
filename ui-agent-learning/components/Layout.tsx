        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
              ...
              {NAV_ITEMS.map((item) => (
                <Link
                  ...
                  className={`flex items-center gap-1 hover:text-primary transition ${
                    location.pathname === item.path ? 'text-primary' : 'text-gray-600 dark:text-gray-300'
                  }`}
                >
            ...
            <div className="flex items-center gap-4">
              <Link to="/cart" className="relative group text-gray-600 dark:text-gray-300 hover:text-primary">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                {NAV_ITEMS.map(item => (
                  <li key={item.path}>
                    <Link to={item.path} className="hover:text-primary flex items-center gap-2">
                      <span className="material-icons-round text-xs">{item.icon}</span> {item.label}
                    </Link>
                  </li>
                ))}
                <li><a href="#" className="hover:text-primary flex items-center gap-2"><span className="material-icons-round text-xs">local_shipping</span> معلومات التوصيل</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold text-lg mb-6 text-accent-brown dark:text-white">تواصل معنا</h4>
              <ul className="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                <li className="flex items-center gap-2"><span className="material-icons-round text-xs">email</span> qanades@gmail.com</li>
